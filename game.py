...
class Game:
    ...
    async def normal(self, bet):
        ...
        winner = await self.determine(self.player_rolls)
        loser = await self.determine(self.player_rolls)

        await self.bot.say("{} owes {} {}g".format(self.get_name(loser), self.get_name(winner), bet))
        loser_and_winner = await self.determine(self.player_rolls)
        await self.bot.say("{} owes {} {}g".format(self.get_name(loser_and_winner[0][0]),
                                                   self.get_name(loser_and_winner[1][0]), bet))
    ...
    async def difference(self, bet):
        ...
        while len(self.players) > len(self.player_rolls):
            if bet == 100:
                ...
                await self.bot.wait_for_message(content='/roll {}'.format(bet), channel=self.channel)
            await asyncio.sleep(0.01)
            if last_roll[0].roller in self.players and last_roll[0].roller not in self.player_rolls:
                self.player_rolls.append((last_roll[0].roller, last_roll[0].rolled))
        ...
        await self.bot.say("{} owes {} {}g".format(self.get_name(loser_and_winner[0][0]),
                                                   self.get_name(loser_and_winner[1][0]), the_difference))
    ...
    async def add(self, player : discord.member.Member):
        if self.in_progress:
            await self.bot.say("It's too late to join")
        else:
