...
class BlackjackDealer:
    ...
    async def show_face_up(self):
        ...
        await self.announcer.dealer_card(first_card, self.game.ctx)
    ...
    async def make_move(self) -> None:
        await self.announcer.dealer_turn(self._hand, self.game.ctx)
    ...
    async def __hit_loop(self) -> None:
        ...
        while self._hand.get_value() < 17:
            ...
            self._hand.hit(card)
            await self.announcer.dealer_hit(card, self.game.ctx)
    ...
    async def __check_ace_or_ten(self) -> bool:
        ...
        if is_ace or is_ten:
            await self.announcer.ace_or_ten_message(self._hand, self.game.ctx)
            return True
    ...
    async def __end_turn(self) -> None:
        ...
        if not is_blackjack and not is_bust:
            await self.announcer.dealer_stand(self.game.ctx)

    async def __check_blackjack(self) -> bool:
        if self._hand.is_blackjack():
            await self.announcer.announce_blackjack(self.game.ctx)
            return True

    async def __check_bust(self) -> bool:
        if self._hand.is_bust():
            await self.announcer.declare_dealer_bust(self.game.ctx)
            return True
